#ifndef ASSETS_GLSL_HELPERS_GLSL
#define ASSETS_GLSL_HELPERS_GLSL
...
// Assumes pos is relative to the center of the circle
float3 circleFunc(float2 pos, float radius, float blur, float thickness) {
    float innerRadius = radius - blur - thickness - blur;

    float dist = length(pos);

    float3 inner = float3(1,0,0);
    float3 outer = float3(0,1,0);
    float3 superOuter = float3(0,0,1);
    //float edge = interpolate(.5, 1, dist )
    if (dist < innerRadius) {
        return inner;
    } else if (dist < innerRadius + blur) {
        return lin(inner, outer, (dist - innerRadius)/blur);
    } else if (dist < innerRadius + blur + thickness) {
        return outer;
    } else if (dist < innerRadius + blur + thickness + blur) {
        return lin(outer, superOuter, (dist - innerRadius - blur - thickness)/blur);
    } else {
        return superOuter;
    }
}
